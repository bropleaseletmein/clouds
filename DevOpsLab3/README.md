...
папйлайны будут:
- собирать докер-образ, в докерфайле будут прописаны все шаги (по невероятным стечениям обсоятельств взял докер-файл со второй лабы) - восстановление зависимостей, сама сборка
...
- все стадии в одно. нет разделения
  - стадия билда, стадия деплоя находятся в одной джобе
  - их надо разнести по двум разным
- все параметры явно захаркожены.
  - порты
  - имя конейнера
  - имя образа
  - тэг образа
    
  все эти вещи в исправленном варианте я буду хранить в GitHub Secrets
- не знаю, насколько правильно это исправлять и насколько это является проблемой, но все же запишу это в проблемы
  - когда мы собираем образ `docker build -t <имя> .` то происходит следующее (насколько я понял) - старый образ с таким же именем и тэгом остается, просто имя и тэг у него превращаются в NONE. после этого добавляется новый. и если посмотреть в образы на нашей ВМ, то можно увидеть вот такой ужас (как мне кажется). в исправленном варианте я буду это чистить
    
    <img width="858" height="292" alt="image" src="https://github.com/user-attachments/assets/b7dd1c1d-5503-495a-b83b-2aac9d706e9f" />

- нет стадии тестов
- нет проверки ошибок при остановке / удалении контейнера

руководствуясь написанным выше попробуем написать +- нормальный ci/cd пайплайн

у меня получилось что-то такое

<img width="748" height="1095" alt="image" src="https://github.com/user-attachments/assets/015ed91e-8d43-42a7-ba03-8887a6877c39" />

разьерем что исправили:
- добавил тест стадию. она идет уже не self-hosted а на раннере гитхаба. и так же она идет перед созданием контейнера. я не хочу зависеть от наличия dotnet на сервере поэтому запускаемся на раннере гитхаба. ну и смысла собирать образ с заведомо неправильным (вообще не билдится / не проходит тесты) кодом особо нет
- теперь все секретики будут в GitHub Secrets - нет явного хардкода
  <img width="788" height="338" alt="image" src="https://github.com/user-attachments/assets/9f223037-fb69-4dd9-a2fd-bfa58ab5f4de" />
